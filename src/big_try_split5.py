...
def _train_val_split(imgs_train, imgs_mask_train):
    ...
    return StratifiedKFold(mean_mask, n_folds=5, shuffle=True, random_state=2016)
...
def train_and_predict():
    ...
    skf = _train_val_split(imgs_train, imgs_mask_train)

    for train_index, test_index in skf:

        X_train = imgs_train[train_index]
        y_train = imgs_mask_train[train_index]

        X_val = imgs_train[test_index]
        y_val = imgs_mask_train[test_index]
        ...
        np.save('split5/imgs_mask_test_{suffix}.npy'.format(suffix=suffix), imgs_mask_test)
