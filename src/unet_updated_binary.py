...
def get_unet():
    ...
    model.compile(optimizer=Adam(lr=1e-7), loss='hinge', metrics=['accuracy', 'binary_crossentropy'])
...
def _train_val_split(imgs_train, imgs_mask_train):
    ...
    X_train, X_test, y_train, y_test = train_test_split(imgs_train, mean_mask, stratify=mean_mask, test_size=0.2)
...
def train_and_predict():
    ...
    X = imgs_train
    y = (np.mean(imgs_mask_train, (2, 3)) == 0).astype(int).flatten().astype(np.float32)
